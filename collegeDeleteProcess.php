...
if (isset($_GET['college_code'])) {
    ...
    try {
        ...
        if ($e->getCode() == 1451) {
            $error = "Cannot delete this college because it has programs linked to it. Please delete those programs first.";
        } else {
            $error = $e->getMessage();
        }
    }
} else {
    $error = "No college code specified.";
}
?>
...
<head>
    ...
    <title><?php echo $success ? "College Deleted" : "Error"; ?></title>
    <style>
        .modal {
            background: white;
            border-radius: 8px;
...
<div class="modal">
    ...
    <?php if ($success): ?>
        <p>College deleted successfully!</p>
    <?php else: ?>
        <p class="error-detail"><?= $error ?></p>
    <?php endif; ?>

    <div class="btn-group">
        <a href="colleges.php" class="btn-back <?= $success ? 'success' : 'error' ?>">Go Back</a>
    </div>
</div>
